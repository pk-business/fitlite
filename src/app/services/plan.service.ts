...
export class PlanService {
  ...
  async generateWorkoutPlan(profile: UserProfile): Promise<WorkoutPlan> {
    ...
    const workoutDays = this.getWorkoutDaysByActivityLevel(profile.activityLevel);
   ...
   */
  private getWorkoutDaysByActivityLevel(activityLevel: string): { dayOfWeek: number; focus: 'full' | 'upper' | 'lower' | 'cardio' }[] {
   ...
   */
  private getExercisesForFocus(focus: 'full' | 'upper' | 'lower' | 'cardio', profile: UserProfile): Exercise[] {
    ...
    switch (focus) {
      case 'full':
        return [
          { name: 'Push-ups', ...setsRepsConfig, restSeconds: 60, category: 'full' },
          { name: 'Bodyweight Squats', ...setsRepsConfig, restSeconds: 60, category: 'full' },
          { name: 'Plank', sets: 3, reps: 1, restSeconds: 60, category: 'full' }, // Duration-based
          { name: 'Lunges', ...setsRepsConfig, restSeconds: 60, category: 'full' },
          { name: 'Dumbbell Rows', ...setsRepsConfig, restSeconds: 60, category: 'full' }
        ];
      
      case 'upper':
        return [
          { name: 'Push-ups', ...setsRepsConfig, restSeconds: 60, category: 'upper' },
          { name: 'Pike Push-ups', ...setsRepsConfig, restSeconds: 60, category: 'upper' },
          { name: 'Dumbbell Rows', ...setsRepsConfig, restSeconds: 60, category: 'upper' },
          { name: 'Dumbbell Bicep Curls', ...setsRepsConfig, restSeconds: 60, category: 'upper' },
          { name: 'Tricep Dips', ...setsRepsConfig, restSeconds: 60, category: 'upper' }
        ];
      
      case 'lower':
        return [
          { name: 'Bodyweight Squats', ...setsRepsConfig, restSeconds: 60, category: 'lower' },
          { name: 'Lunges', ...setsRepsConfig, restSeconds: 60, category: 'lower' },
          { name: 'Bulgarian Split Squats', ...setsRepsConfig, restSeconds: 60, category: 'lower' },
          { name: 'Glute Bridges', ...setsRepsConfig, restSeconds: 60, category: 'lower' },
      ...
      case 'cardio':
        return [
          ...
          { name: 'Jump Rope', sets: 3, reps: 1, restSeconds: 60, category: 'cardio' } // Duration-based
        ];
   ...
   */
  private getSetsRepsForGoal(goal: 'lose' | 'maintain' | 'gain'): { sets: number; reps: number } {
    switch (goal) {
      case 'lose':
  ...
  async generateDietPlan(profile: UserProfile): Promise<DietPlan> {
    const targetCalories = this.userProfileService.calculateTargetCalories(profile);
   ...
   */
  private calculateMacros(calories: number, goal: 'lose' | 'maintain' | 'gain'): { protein: number; carbs: number; fat: number } {
    ...
    switch (goal) {
      case 'lose':
        ...
        break;
      case 'gain':
        proteinPercent = 0.30; // High protein for muscle gain
        fatPercent = 0.25;
   ...
   */
  private generateDailyMeals(targetCalories: number, macros: { protein: number; carbs: number; fat: number }): Meal[] {
    ...
    const calDinner = Math.round(targetCalories * 0.30);
    const calSnacks = Math.round(targetCalories * 0.10);
   ...
   */
  async generateAllPlans(profile: UserProfile): Promise<{ workout: WorkoutPlan; diet: DietPlan }> {
  ...
  async updateWorkoutDay(dayOfWeek: number, day: WorkoutDay): Promise<void> {
    ...
    const dayIndex = plan.weeklyPlan.findIndex(d => d.dayOfWeek === dayOfWeek);
    if (dayIndex === -1) {
   ...
   */
  async removeExerciseFromDay(dayOfWeek: number, exerciseIndex: number): Promise<void> {
    const plan = await this.getWorkoutPlan();
    if (!plan) {
      throw new Error('No workout plan found');
    }
   ...
   */
  async updateExerciseInDay(dayOfWeek: number, exerciseIndex: number, exercise: Exercise): Promise<void> {
    const plan = await this.getWorkoutPlan();
    if (!plan) {
      throw new Error('No workout plan found');
    }
   ...
   */
  async reorderExercises(dayOfWeek: number, exercises: Exercise[]): Promise<void> {
    const plan = await this.getWorkoutPlan();
    if (!plan) {
      throw new Error('No workout plan found');
    }
